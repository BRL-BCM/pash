...
//***** printUsage
void printUsage() {fprintf(stderr,"\nmakeIgnoreList.exe\nReads key frequency table generated by keyFreq.exe (in binary format) and\n    outputs 'ignore list' file for Pash.  Keys with occurance > specified\n    maxCutoff or < specified minCutoff are ignored.  May only specify a maximum or minimum, in which case the other is ignored.  Ignore list output is a bit vector, where '1' means ignore\n    and '0' means don't ignore.\n\nUsage: makeIgnoreList.exe -i <inputFile> -o <outputFile> -c <max frequency cutoff> -m <min frequency cutoff>\n");}
//***** printUsage
...
void processFile(FILE *inputFile, FILE *outputFile)
  ...
  while(!feof(inputFile))
    ...
    for(ii=0; ii<readBuff.content; ii++)
      ...
      if((minCutoff!=-1 && readBuff.buff[ii] < minCutoff) || (maxCutoff!=-1 && readBuff.buff[ii] > maxCutoff))
	     readBuff.buff[ii]=1;
	else readBuff.buff[ii]=0;
      if(debug) if( readBuff.buff[ii]) {
				fprintf(stderr,"key %i ignored\n", total-1);
			} else {
			  	fprintf(stderr,"key %i considered\n", total-1);
			}
  ...
  if(inferPatternWeight(total, &ii))
    fprintf(stderr,"Warning: %i total keys processed -- not a power of 4.  Maybe input is wrong?\n", total);
  else if(debug) fprintf(stderr,"%i total keys processed.  Inferred key length is %i\n",total, ii);
